/*
Electron inherits from Particle
 */
...
class Electron extends Particle {
  ...
  Electron(float _x, float _y, float _z, float _r, float _m, PVector _v, int _t, PVector _f, float _c) { 
  ...
  PVector calculateForceforProton(PVector jpos) {
    ...
    if (r.mag() <= 2*15) {
      ...
      force = runit.mult(G * mass * 1 / pow(r.mag(), 2));
    }
    return force;
  };
  ...
  PVector calculateForceforElectron(PVector jpos) {
    ...
    if (r.mag() <= 2*15) {
      ...
      //force = runit.mult(-G * mass * 0.545 / pow(r.mag(), 2));
      force = runit.mult((-G * mass * 1 / pow(r.mag(), 2)));
    }
    return force;
  };
